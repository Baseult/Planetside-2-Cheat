...
void ESP::Render() {
    ...
    // Sortiere Entities nach Distanz (von weit nach nah), damit sich Boxen korrekt �berlappen
    ...
    for (const auto& entity : sortedEntities) {
        ...
        if (isTarget) continue; // Ziele werden sp�ter gezeichnet

        float alpha = 1.0f;
        if (IsPlayerType(entity.type)) DrawPlayerESP(entity, entity.feetScreenPos, entity.headScreenPos, alpha);
        else if (IsGroundVehicleType(entity.type)) DrawGroundVehicleESP(entity, entity.feetScreenPos, alpha);
        else if (IsAirVehicleType(entity.type)) DrawAirVehicleESP(entity, entity.feetScreenPos, alpha);
        else if (IsTurretType(entity.type)) DrawTurretESP(entity, entity.feetScreenPos, alpha);
        else if (IsOthersType(entity.type)) DrawOthersESP(entity, entity.feetScreenPos, alpha);
        m_renderedEntities++;
    }
    ...
    if (targetOpt) {
        ...
        if (target.isAlive && target.isOnScreen) {
            float alpha = 1.0f;
            if (IsPlayerType(target.type)) DrawPlayerESP(target, target.feetScreenPos, target.headScreenPos, alpha);
            else if (IsGroundVehicleType(target.type)) DrawGroundVehicleESP(target, target.feetScreenPos, alpha);
            else if (IsAirVehicleType(target.type)) DrawAirVehicleESP(target, target.feetScreenPos, alpha);
            else if (IsTurretType(target.type)) DrawTurretESP(target, target.feetScreenPos, alpha);
            else if (IsOthersType(target.type)) DrawOthersESP(target, target.feetScreenPos, alpha);
             m_renderedEntities++;
        }
    }
...
bool ESP::IsCurrentTarget(const EntitySnapshot& entity) const {
    auto currentTargetOpt = g_TargetManager->GetCurrentTarget();
    if (!currentTargetOpt) return false;
    return currentTargetOpt->id == entity.id;
}

void ESP::DrawPlayerESP(const EntitySnapshot& entity, const Utils::Vector2& feetScreenPos, const Utils::Vector2& headScreenPos, float alphaMultiplier) {
    ...
    if (g_Settings.ESP.bBoxes) {
        // Diagonale Box: Berechne die vier Ecken basierend auf Kopf- und Fu�positionen
        const Utils::Vector2 topLeft = { headScreenPos.x - boxWidth / 2.0f, headScreenPos.y };
        const Utils::Vector2 topRight = { headScreenPos.x + boxWidth / 2.0f, headScreenPos.y };
        const Utils::Vector2 bottomLeft = { feetScreenPos.x - boxWidth / 2.0f, feetScreenPos.y };
        const Utils::Vector2 bottomRight = { feetScreenPos.x + boxWidth / 2.0f, feetScreenPos.y };
        DrawDiagonalBox(entity, topLeft, topRight, bottomLeft, bottomRight, alphaMultiplier);
    }
    if (g_Settings.ESP.bHealthBars) {
        // Diagonale Health Bar: Berechne die Positionen basierend auf Kopf- und Fu�positionen
        const Utils::Vector2 topLeft = { headScreenPos.x - boxWidth / 2.0f, headScreenPos.y };
        const Utils::Vector2 topRight = { headScreenPos.x + boxWidth / 2.0f, headScreenPos.y };
        const Utils::Vector2 bottomLeft = { feetScreenPos.x - boxWidth / 2.0f, feetScreenPos.y };
        const Utils::Vector2 bottomRight = { feetScreenPos.x + boxWidth / 2.0f, feetScreenPos.y };
        DrawDiagonalHealthBar(entity, topLeft, topRight, bottomLeft, bottomRight, alphaMultiplier);
    }
    DrawInfoLabel(entity, alphaMultiplier);
    if (g_Settings.ESP.bSkeletons) DrawSkeleton(entity, feetScreenPos);
    if (g_Settings.ESP.bTracers) DrawTracer(entity, feetScreenPos);
}

void ESP::DrawGroundVehicleESP(const EntitySnapshot& entity, const Utils::Vector2& screenPos, float alphaMultiplier) {
    // Dynamische Gr��e basierend auf der Distanz (�ber die H�he der Box im Snapshot)
...
void ESP::DrawAirVehicleESP(const EntitySnapshot& entity, const Utils::Vector2& screenPos, float alphaMultiplier) {
    float boxHeight = abs(entity.feetScreenPos.y - entity.headScreenPos.y);
    float hexagonSize = boxHeight * 0.5f; // Gr��e anpassen
...
void ESP::DrawTurretESP(const EntitySnapshot& entity, const Utils::Vector2& screenPos, float alphaMultiplier) {
    float boxHeight = abs(entity.feetScreenPos.y - entity.headScreenPos.y);
    float size = boxHeight * 0.8f;
    Utils::Vector2 center = { entity.feetScreenPos.x, entity.headScreenPos.y + boxHeight / 2.0f };
...
void ESP::DrawOthersESP(const EntitySnapshot& entity, const Utils::Vector2& screenPos, float alphaMultiplier) {
    float boxHeight = abs(entity.feetScreenPos.y - entity.headScreenPos.y);
...
void ESP::DrawBox(const EntitySnapshot& entity, const Utils::Vector2& topLeft, float width, float height, float alphaMultiplier) {
    bool isTarget = IsCurrentTarget(entity);
    if (isTarget && g_Settings.ESP.bHighlightTarget) {
...
void ESP::DrawHealthBar(const EntitySnapshot& entity, const Utils::Vector2& topLeft, float width, float height, float alphaMultiplier) {
    // Bar parameters - dynamically scale based on box height
    const float barWidth = height * 0.06f;
    const float barGap = height * 0.04f;
    const float barSpacing = height * 0.02f;
    
    // WICHTIG: Bars immer vertikal ausrichten, unabh�ngig von der Box-Neigung
    // Berechne die vertikale Mitte der Box und positioniere die Bars links davon
    const float boxCenterX = topLeft.x + width / 2.0f;
    const float boxTopY = topLeft.y;
    const float boxBottomY = topLeft.y + height;
    
    // Bars links von der Box, vertikal zentriert
    const Utils::Vector2 barPos = { boxCenterX - barGap - barWidth, boxTopY };
    ...
    // Distanz-basierte Border-Berechnung
    float distance = entity.distanceToLocalPlayer;

    // Border-Farb-Interpolation: Schwarz bei 40m, Bar-Farbe bei 50m
    ...
    if (distance <= 50.0f) {
        if (distance <= 40.0f) {
            ...
            // Lineare Interpolation zwischen 40m und 50m (0.0 = Schwarz, 1.0 = Bar-Farbe)
            colorInterpolation = (distance - 40.0f) / (50.0f - 40.0f); // 0.0 bis 1.0
        }
    }
    else {
        colorInterpolation = 1.0f; // 100% Bar-Farbe bei 50m und weiter
    }

    // Shield bar (upper half) - 50/50 split
    const float shieldBarHeight = (height - barSpacing) * 0.5f;
    // WICHTIG: Immer vertikale Positionierung verwenden
    const Utils::Vector2 shieldBarPos = { barPos.x, boxTopY };

    // Shield bar foreground
    if (shieldPercent > 0.0f) {
        float shieldHeight = shieldBarHeight * shieldPercent;
        // Immer von oben nach unten zeichnen (vertikal)
        ...
        if (alphaMultiplier > 0.0f) {
            // Interpoliere zwischen Schwarz und Cyan basierend auf Entfernung
            float shieldBorderColor[4] = {
                0.0f * (1.0f - colorInterpolation) + 0.0f * colorInterpolation,  // R: Schwarz zu Cyan
                0.0f * (1.0f - colorInterpolation) + 1.0f * colorInterpolation,  // G: Schwarz zu Cyan
                0.0f * (1.0f - colorInterpolation) + 1.0f * colorInterpolation,  // B: Schwarz zu Cyan
                alphaMultiplier
            };
    ...
    const float healthBarHeight = (height - barSpacing) * 0.5f;
    // WICHTIG: Immer vertikale Positionierung verwenden
    const Utils::Vector2 healthBarPos = { barPos.x, boxTopY + shieldBarHeight + barSpacing };
    ...
    if (healthPercent > 0.0f) {
        ...
        // Verbesserte Farben basierend auf Health-Prozent
        float healthColor[4];
        ...
        if (alphaMultiplier > 0.0f) {
            // Interpoliere zwischen Schwarz und Health-Farbe basierend auf Entfernung
...
void ESP::DrawSkeleton(const EntitySnapshot& entity, const Utils::Vector2& screenPos) {
    // Check if this entity is the current target
    bool isTarget = IsCurrentTarget(entity);
    ...
    // Use head and feet positions from the snapshot
    Utils::Vector2 headScreenPos = entity.headScreenPos;
    Utils::Vector2 feetScreenPos = entity.feetScreenPos;

    // Draw circle at head position - size based on distance/box size
    // Calculate dynamic circle radius based on box height (which scales with distance)
    float boxHeight = abs(headScreenPos.y - feetScreenPos.y);
    float circleRadius = boxHeight * 0.08f; // 8% of box height, scales with distance

    g_Renderer->DrawCircle(headScreenPos, circleRadius, skeletonColor, 64, 2.0f);

    // Draw line from head to feet
    g_Renderer->DrawLine(headScreenPos, feetScreenPos, skeletonColor, 2.0f);
}
...
void ESP::DrawInfoLabel(const EntitySnapshot& entity, float alphaMultiplier) {
    ...
    if (IsPlayerType(entity.type))          labelAnchorWorldPos.y -= 0.35f;
    else if (IsGroundVehicleType(entity.type)) labelAnchorWorldPos.y -= 1.35f;
    else if (IsAirVehicleType(entity.type))    labelAnchorWorldPos.y -= 3.0f;
    else if (IsTurretType(entity.type))        labelAnchorWorldPos.y -= 0.5f;
    else if (IsOthersType(entity.type))        labelAnchorWorldPos.y -= 0.2f;
    else                                    labelAnchorWorldPos.y -= 0.35f; // Default fallback
